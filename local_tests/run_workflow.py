...
def load_configs() -> tuple[Customer, dict]:
    ...
    cfg_files = os.listdir(config_path)

    customers = []
    for cfg_file in cfg_files:
        if cfg_file.endswith(".json") and cfg_file != "MainConfig.json":
            with open(config_path / cfg_file, encoding="utf-8") as fh:
                cust_cfg_raw = json.load(fh)
            cfg = CustomerConfig(
                base_columns=main_cfg["base_columns"], **cust_cfg_raw)
            customers.append(Customer(cfg))

    return customers, main_cfg["base_columns"]
...
def run():
    ...
    customers, base_columns = load_configs()
